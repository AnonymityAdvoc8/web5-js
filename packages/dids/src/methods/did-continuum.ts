...
//  */
// export interface DidContinuumCreateOptions<TKms> extends DidCreateOptions<TKms> {
//   /**
 ...
 * ```
 */
export interface DidContinuumVerificationMethod {
  /**
 ...
 */
const DEFAULT_GATEWAY_URI = 'https://resolver.continuum.network';
...
export class DidContinuum extends DidMethod {

  /**
   * Name of the DID method, as defined in the DID ION specification.
   */
  public static methodName = 'continuum';

  /**
   * Creates a new DID using the `did:continuum` method formed from a newly generated key.
   *
   * Notes:
...
//   }
  ...
  /**
   * Instantiates a {@link BearerDid} object for the DID CONTINUUM method from a given {@link PortableDid}.
   *
   ...
   * const did = await DidContinuum.import({ portableDid });
   * ```
   *
  ...
  public static async import({ portableDid, keyManager = new LocalKeyManager() }: {
    ...
    if (parsedDid?.method !== DidContinuum.methodName) {
...
//   }
  ...
  /**
   * Resolves a `did:continuum` identifier to its corresponding DID document.
   *
   ...
   * @example
   * ```ts
   * const resolutionResult = await DidContinuum.resolve('did:ion:example');
   * ```
   *
  ...
  public static async resolve(didUri: string, options: DidResolutionOptions = {}): Promise<DidResolutionResult> {
    ...
    try {
      ...
      const resolutionUrl = DidContinuumUtils.appendPathToUrl({
        baseUrl : gatewayUri,
        path    : `/did/${didUri}`
      });
 ...
 */
export class DidContinuumUtils {
  /**
